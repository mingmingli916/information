...
There are also two functions =1+= and =1-=, which return their argument plus 1 and minus 1 respectively. The name =1-= is a bit misleading, because =(1- x)= returns =x - 1=,not =1 - x=. (-1 can not be a function name.)
...
function referred to in the =car=, evaluate all the arguments in the =cdr=, and return the result of applying the former to the latter.
...
Auxiliary methods are defined by putting a quilifiying keyword after after the method name in the call to =defmethod=. If we define a primary =speak= method for the =speaker= class as:
#+begin_src lisp
  (defclass speaker () ())
...
#+begin_src lisp
  (defmethod speak :before ((s speaker) string)
    (princ "I think "))
#+end_src
they will get called in teh middle of the sandwich:
#+begin_src lisp
CL-USER> (speak (make-instance 'intellectual)
                "I'm hungry")
